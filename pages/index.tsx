...
const Index: any = () => {
  ...
  React.useEffect(() => {
    const onResize = () => {
      if (baseContainerRef.current && containerRef.current && bodyRef.current) {
        ...
        containerRef.current.style.height = `${bodyRef.current.offsetHeight * scale}px`
      }
    }
  ...
  return (
    ...
    <div ref={baseContainerRef} className="flex flex-col items-center">
      <div className="mt-8 flex space-x-2 items-center" style={{width: '100%', maxWidth: 820}}>
      ...
      <div className="mt-4">
        <LoadingButton onClick={() => onDownload(true)} loading={copying} disabled={loading || copying || downloading}>Copy</LoadingButton>
        <LoadingButton onClick={() => onDownload(false)} loading={downloading} disabled={loading || copying || downloading}>Download</LoadingButton>
      </div>
    </div>
      </div>
    </div>
